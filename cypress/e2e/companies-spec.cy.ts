...
describe('abcjobs - companies', () => {
  ...
  context('Given an user company accesses abcjobs register page', () => {
    beforeEach(() => {
      cy.visit('http://localhost:3000/es/auth/register/company')
      // cy.wait(1000)
    })

    context('When user company see register page', () => {
      it('Then he/she sees register company form', () => {
        cy.checkCompanyTest()
      })
    })

    context('When user company see register page, no input data and click submit button', () => {
      it('Then he/she sees error messages', () => {
        cy.createCompanyTestEmpty()
      })
    })

    context('When user company see register page, input all data and click submit button', () => {
